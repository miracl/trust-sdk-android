 ...
 */
public class MIRACLTrustCore private constructor(
    context: Context,
    ...
    public companion object {
        ...
        private lateinit var instance: MIRACLTrustCore

        @JvmStatic
        public fun getInstance(): MIRACLTrustCore =
         ...
         */
        @JvmStatic
        public fun configure(context: Context, configuration: ConfigurationCore) {
            instance = MIRACLTrustCore(context, configuration)
        }
    }
    ...
    public fun getAuthenticationSessionDetailsFromAppLink(
        ...
        miraclTrustScope.launch {
            sessionManager.getSessionDetailsFromAppLink(appLink, projectUrl)
                .also { result ->
    ...
    public fun getAuthenticationSessionDetailsFromQRCode(
        ...
        miraclTrustScope.launch {
            sessionManager.getSessionDetailsFromQRCode(
                qrCode = qrCode,
    ...
    public fun getAuthenticationSessionDetailsFromNotificationPayload(
        ...
        miraclTrustScope.launch {
            sessionManager.getSessionDetailsFromNotificationPayload(payload, projectUrl)
                .also { result ->
                    if (result is MIRACLError) {
                        logError(
                            LoggerConstants.SESSION_MANAGER_TAG,
    ...
    public fun abortAuthenticationSession(
        ...
        miraclTrustScope.launch {
            sessionManager.abortSession(authenticationSessionDetails, projectUrl).also { result ->
                if (result is MIRACLError) {
                    logError(
                        LoggerConstants.SESSION_MANAGER_TAG,
                        result.value
    ...
    public fun getCrossDeviceSessionFromAppLink(
        ...
        miraclTrustScope.launch {
            crossDeviceSessionManager.getCrossDeviceSessionFromAppLink(appLink, projectUrl)
                .also { result ->
                    if (result is MIRACLError) {
                        logError(
    ...
    public fun getCrossDeviceSessionFromQRCode(
        ...
        miraclTrustScope.launch {
            crossDeviceSessionManager.getCrossDeviceSessionFromQRCode(
                qrCode = qrCode,
                projectUrl = projectUrl
            ).also { result ->
                if (result is MIRACLError) {
    ...
    public fun getCrossDeviceSessionFromNotificationPayload(
        ...
        miraclTrustScope.launch {
            crossDeviceSessionManager.getCrossDeviceSessionFromNotificationPayload(
                payload = payload,
                projectUrl = projectUrl
            ).also { result ->
                if (result is MIRACLError) {
    ...
    public fun abortCrossDeviceSession(
        ...
        miraclTrustScope.launch {
            crossDeviceSessionManager.abortSession(crossDeviceSession, projectUrl).also { result ->
                if (result is MIRACLError) {
                    logError(
                        LoggerConstants.SESSION_MANAGER_TAG,
                        result.value
    ...
    public fun getActivationToken(
        ...
        miraclTrustScope.launch {
            verificator.getActivationToken(
                userId = userId, code = code, projectUrl = projectUrl
            ).also { result ->
                if (result is MIRACLError) {
                    logError(
                        LoggerConstants.VERIFICATOR_TAG,
     ...
     */
    public suspend fun getUser(userId: String, projectId: String): User? {
        return withContext(Dispatchers.IO) {
            try {
    ...
    public fun getUser(
        userId: String,
        projectId: String,
        resultHandler: ResultHandler<User?, UserStorageException>
    ) {
        miraclTrustScope.launch {
            try {
